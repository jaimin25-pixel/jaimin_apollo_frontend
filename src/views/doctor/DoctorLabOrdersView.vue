<template>
  <div class="lab-orders">
    ...
    <div v-else-if="doctorStore.labOrders.length === 0" class="no-data">
      <p>No lab orders yet</p>
    </div>

    <div v-else class="orders-list">
      <div v-for="order in doctorStore.labOrders" :key="order.id" class="order-card">
        ...
        <div class="card-body">
          ...
          <div class="detail-row">
            ...
            <span>{{ order.test_names?.join(', ') || 'N/A' }}</span>
          </div>
          ...
          <div v-if="order.result_date" class="detail-row">
            <span class="label">Result Date:</span>
            <span>{{ formatDate(order.result_date) }}</span>
          </div>
        </div>
    ...
    <div v-if="selectedOrder" class="modal-overlay" @click="selectedOrder = null">
      <div class="modal-content" @click.stop>
        ...
        <div class="detail-section">
          <h4>Tests</h4>
          <ul>
            <li v-for="(test, idx) in selectedOrder.test_names" :key="idx">{{ test }}</li>
          </ul>
        </div>
        ...
        <div v-if="selectedOrder.result_date" class="detail-section">
          <h4>Results</h4>
          <p><strong>Result Date:</strong> {{ formatDate(selectedOrder.result_date) }}</p>
        </div>
...
onMounted(async () => {
  await doctorStore.loadLabOrders()
})

const viewOrder = async (orderId: number) => {
  selectedOrder.value = await doctorStore.fetchLabOrder(orderId)
}

const handleCreateOrder = async () => {
  if (!createForm.value.patient_id || testNamesText.value.trim() === '') {
    return
  }

  const testNames = testNamesText.value.split(',').map((t) => t.trim())

  try {
    await doctorStore.createLabOrder({
